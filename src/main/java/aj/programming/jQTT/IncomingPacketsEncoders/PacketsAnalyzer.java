...
import aj.programming.jQTT.Core.PacketAnalyzerRunnable;
import aj.programming.jQTT.IncomingPacketsEncoders.Enums.MessageType;
import aj.programming.jQTT.IncomingPacketsEncoders.FixedHeaders.MqttFixedFixedHeaderDec;
import aj.programming.jQTT.IncomingPacketsEncoders.Payload.*;
import aj.programming.jQTT.IncomingPacketsEncoders.VariableHeader.*;
...
public class PacketsAnalyzer implements PacketAnalyzerRunnable {
    ...
    public void run() {
        ...
        try {
            while (running) {
                if (packetsBuffer.getSize() > 0) {
                    if (!this.processingFlag) {
                        ...
                        this.packetData.setRemainingLen(remainingLength);
            ...
            logger.warning(String.format("PacketAnalyzer interrupted, message: %s, stack:", e.getMessage(), Arrays.toString(e.getStackTrace())));
        }
    }
    ...
    public class PacketData {
        ...
        public void setRemainingLen(int remainingLen) {
            this.remainingLen = remainingLen;
        }
